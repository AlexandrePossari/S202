...
Contexto: Imagine que você está desenvolvendo um banco de dados de cache para uma rede social. O objetivo do sistema
 é sempre exibir os posts mais recentes dessa rede social em ordem de interesse. 
Para isso, são mantidas informações sobre os interesses dos usuários e sobre o tópico de cada postagem.
Esse sistema deve armazenar o perfil de cada usuário, contendo o nome, o e-mail e um token de sessão. 
Além, disso o sistema deve manter uma lista de interesses para cada usuário, ordenando pelo score de interesse.
Por fim, o sistema deve manter informações sobre os posts, como id, conteúdo, e-mail do autor e data e hora de publicação.
 É registrado também uma lista de palavras chave sobre o post. Apenas as informações dos posts mais recentes 
 devem ser mantidos no banco de dados (posts com mais de 5 horas devem ser excluídos). 
Considere que o interesse de um usuário por um post específico é a 
soma dos scores de cada interesse que está presente nas palavras chave desse post específico.
...
def questao_1(users):
    ...
    redis_conn.hset("user:2", mapping={
        "nome": "Tamara Borges",
        "email": "tam_borges@g.com"
    })
    redis_conn.hset("user:3", mapping={
        "nome": "Ubiratã Carvalho",
        "email": "bira@g.com"
    })
    redis_conn.hset("user:4", mapping={
        "nome": "Valéria Damasco",
        "email": "valeria_damasco@g.com"
    })

    redis_conn.lpush("users", "user:1","user:2","user:3","user:4")

    numero_usuarios = redis_conn.llen("users")

    users = redis_conn.lrange("users", 0, numero_usuarios-1)
    print(users)
    for user in users:
        print(redis_conn.hget(user, "nome"))
        print(redis_conn.hget(user, "email"))
        print()
    pass
    return users
...
def questao_2(interests):
    redis_conn.lpush("interests1", {"futebol":0.855}, {"pagode":0.765}, {"engraçado":0.732}, {"cerveja":0.622}, {"estética":0.519})
    redis_conn.lpush("interests2", {"estética":0.765}, {"jiujitsu":0.921}, {"luta":0.884}, {"academia":0.541}, {"maquiagem":0.658})
    redis_conn.lpush("interests3", {"tecnologia":0.999}, {"hardware":0.865}, {"games":0.745}, {"culinária":0.658}, {"servers":0.54})
    redis_conn.lpush("interests4", {"neurociências":0.865}, {"comportamento":0.844}, {"skinner":0.854}, {"laboratório":0.354}, {"pesquisa":0.428})

    n_interests = redis_conn.llen("interests1")
    interests1 = redis_conn.lrange("interests1", 0, n_interests-1)
    n_interests = redis_conn.llen("interests2")
    interests2 = redis_conn.lrange("interests2", 0, n_interests-1)
    n_interests = redis_conn.llen("interests3")
    interests3 = redis_conn.lrange("interests3", 0, n_interests-1)
    n_interests = redis_conn.llen("interests4")
    interests4 = redis_conn.lrange("interests4", 0, n_interests-1)
    pass

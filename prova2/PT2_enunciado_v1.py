...
cloud_config= {
  'secure_connect_bundle': 'secure-connect-prova2.zip'
}

# This token JSON file is autogenerated when you download your token, 
# if yours is different update the file name below
with open("prova2-token.json") as f:
    secrets = json.load(f)
...
cassandra_session.set_keyspace('p2') # Change to your keyspace
...
def questao_1_a(users):
    cassandra_session.execute("DROP TABLE IF EXISTS usuario;")
    query = ''' 
        CREATE TABLE usuario(
            ...
            email text,
            interesses text,
            primary key((nome), endereco, id)
        );
    '''
    cassandra_session.execute(query)
    cassandra_session.execute("TRUNCATE usuario;")

    cassandra_session.execute("DROP TABLE IF EXISTS produto;")
    query = ''' 
    ...
    for user in users:
        query = """ INSERT INTO usuario(id, estado, cidade, endereco, nome, email, interesses) VALUES (%s, %s, %s, %s, %s, %s, %s) """
...
def questao_1_b(products):
    for product in products:
        query = """ INSERT INTO produto(id, categoria, nome, custo, preco, quantidade) VALUES (%s, %s, %s, %s, %s, %s) """
        cassandra_session.execute(query, (product["id"], product["categoria"], product["nome"], int(product["custo"]*100), int(product["preco"]*100), product["quantidade"]))

    query = """ SELECT custo, quantidade FROM produto; """
    result = cassandra_session.execute(query).one()

    custo = 0
    for product in result:
        custo = custo + (product["custo"] * product["quantidade"] / 1000)
    pass
    return custo
...
# Questão 2
def questao_2(state):
    query = """ SELECT * FROM usuario WHERE estado = %s ALLOW FILTERING; """
    result = cassandra_session.execute(query, state).all()
...
def questao_3(user_id):
    user_keys = redis_conn.keys(f"interesses:usuario:{user_id}")
    ...
    for interest in interests:
        query = """ SELECT * FROM produto WHERE categoria = %s ALLOW FILTERING; """
        result = cassandra_session.execute(query, interest).all()
